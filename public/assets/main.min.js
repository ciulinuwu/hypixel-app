...
"Wool Games"}return b}function getGame(b){return $gameTypes[b]&&$gameTypes[b].name||getGameList(b)}function getGameMode(b,d){return $gameTypes[b]&&$gameTypes[b].modeNames&&$gameTypes[b].modeNames[d]||d}function addCache(b,d,f){var g=getItem(b);g[d]=f;localStorage.setItem(b,JSON.stringify(g))}function isUUID(b){b=b.replace(/-/g,"");return/^[0-9a-f]{32}$/i.test(b)}$inputSettingsApiKey.on("input",function(){localStorage.setItem("apiKey",$inputSettingsApiKey.val())});
...
$("#lookUp").submit(function(b){b.preventDefault();$buttonLookUpSubmit.attr("data-action-looking",!0);if(2>$inputLookUpUUID.val().length)return errorHandler("Field cannot be empty.");if(!/^[a-zA-Z0-9_-]*$/gm.test($inputLookUpUUID.val()))return errorHandler("Player name must only contain letters, numbers and underscores!");if(getItem("cache")&&getItem("cache")[$inputLookUpUUID.val().toLowerCase()])return getPlayerStats(getItem("cache")[$inputLookUpUUID.val().toLowerCase()]);isUUID($inputLookUpUUID.val())?
getPlayerStats($inputLookUpUUID.val()):getPlayerUUID();return!1});setInterval(getServerStats,12E4);setInterval(getWatchdogStats,12E4);setInterval(setPlayTime);
