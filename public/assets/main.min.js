...
var $inputSettingsApiKey=$("[class='Input.Settings.apiKey']"),$inputLookUpUUID=$("[class='Input.lookUp.UUID']"),$buttonSettingsSApiKey=$("[class^='Button.Settings.s_apiKey']"),$buttonLookUpSubmit=$("[class^='Button.lookUp.submit']"),$buttonSettingsClearCache=$("[class^='Button.Settings.clearCache']"),$gameTypes={},$achievements={},$endpoint="https://api.hypixel.net",$isSetup=!1,$lastLogin=0,$isOnline=!1,$data={};try{getItem("")}catch(b){$alert("Cookies are disabled","Local Storage is required for the app to function correctly.")}
$(document).ready(function(){$("body").removeAttr("style");$("#globalPop").popup();setup_chk();getServerStats();getGameTypes();getAchievements()});function getItem(b){b=localStorage.getItem(b);try{return JSON.parse(b)}catch(d){return b}}$inputSettingsApiKey.val(getItem("apiKey"));
function $alert(b,d,f,g){d=d.replace(/(\r\n|\r|\n)/g,"<br/>");$("#globalPop .title span").text(b);$("#globalPop .text span").html(d);var h=function(){$("#globalPop").popup("close");$("#globalPop button").unbind("click")},k=setInterval(function(){$("#globalPop").popup("open");clearInterval(k);1===g&&$("#globalPop button.btn-b").removeAttr("hidden");1!==g&&$("#globalPop button.btn-b").attr("hidden","");$("#globalPop button.btn-a").on("click","function"===typeof f&&f||h);$("#globalPop button.btn-b").on("click",
h)},1)}function app_run(){getWatchdogStats()}function setup_run(){$isSetup=!0;localStorage.setItem("cache",JSON.stringify({}));$alert("Welcome!","Please enter API key to continue.",function(){$("#settings a.revert-btn").hide();$(".OTHERSETTINGS").hide();$.mobile.changePage("#settings")})}function setup_chk(){getItem("setup")?app_run():setup_run()}
...
g+".svg')></div></a>"}).appendTo($("[data-name='"+f+"'] .achievements")),$("[data-internal='"+f+"_"+g.toLowerCase()+"']").click(ent_achiv_click)}}function ent_achiv_click(b){b=b.currentTarget.dataset;var d="true"==b.completed;$("#achievementPop").html("<p>"+b.name+'</p><p style="font-size: 14px">'+b.description+'</p><hr/><p class="center" style="font-size: 12px" data-text-color="'+(d?"GREEN":"GRAY")+'">'+(d?"Completed":"Not Completed")+"</p>")}
...
function parsePlayerStats(b){clearField();clearAchiev();$buttonLookUpSubmit.attr("data-action-looking",!1);$lastLogin=b._default.lastLogin;$isOnline=b._recentgame.online;var d=b._default.prefix||b._default.rank||("NONE"!==b._default.monthlyPackageRank?b._default.monthlyPackageRank:!1)||b._default.newPackageRank||"DEFAULT",f=b._default.achievementsOneTime;getItem("cache")[b._default.playername.toLowerCase()]||addCache(b._default.playername.toLowerCase(),b._default.uuid);$("[class='Field.Main.PlayerName']").html(buildName(b._default.displayname,
d));$("[class='Field.Main.Rank']").html(buildRank(d,b._default.rankPlusColor||"GOLD"));$("[class='Field.Main.firstLogin']").text(buildTime(b._default.firstLogin));$("[class='Field.Main.lastLogin']").text(buildTime(b._default.lastLogin));$("[class='Field.Main.networkExp']").text(parseNumber(b._default.networkExp));$("[class='Field.Main.karma']").text(parseNumber(b._default.karma||0));$("[class='Field.Main.achievementPoints']").text(parseNumber(b._default.achievementPoints));$("[class='Field.Main.UUID']").text(b._default.uuid);
$("[class='Field.Main.mostRecentGameType']").text($isOnline?getGame(b._recentgame.gameType):"Not currently online");$("[class='Field.recentGame.gameType']").text($isOnline?getGame(b._recentgame.gameType):"Not currently online");$("[class='Field.recentGame.gameMode']").text(getGameMode(b._recentgame.gameType,b._recentgame.mode));$("[class='Field.recentGame.gameMap']").text(b._recentgame.map||"Unknown");$("[class='Field.networkExp.Level").text(getNetworkLevel(b._default.networkExp));$("[class='Field.networkExp.Experience']").text(parseNumber(b._default.networkExp));
$("[class='Field.networkExp.expToNextLevel']").text(parseNumber(getLevelUpExp(getNetworkLevel(b._default.networkExp))));try{for(b=0;b<f.length;++b)$("[data-internal='"+f[b]+"']").attr("class","completed"),$("[data-internal='"+f[b]+"']").attr("data-completed",!0)}catch(g){}$.mobile.changePage("#stats:player")}function getPlayerStatus(b){_fetch("GET",$endpoint+"/status?key="+getItem("apiKey")+"&uuid="+b,function(d){$data._recentgame=d.session;parsePlayerStats($data)},errorHandler)}
function clearField(){$("[data-role='page']:not([id='stats:server']) .ui-field > p > small").each(function(){$(this).text("")})}function clearAchiev(){$("[data-completed='true']").attr("data-completed",!1);$("li.completed").removeAttr("class")}function _fetch(b,d,f,g){$.ajax({type:b,url:d,success:f,error:g})}function validateApiKeyRegex(b){return/[a-z0-9][a-z0-9]*-[a-z0-9][a-z0-9]*-[a-z0-9]/.test(b)}function addZero(b){10>b&&(b="0"+b);return b}
function sortABC(b,d){return b.innerHTML.toLowerCase()>d.innerHTML.toLowerCase()?1:-1}function addS(b,d){return 1<d?d+" "+b+"s":d+" "+b}function nHidden(){return"Hidden"}function AMorPM(b){return 12>b?"AM":"PM"}function parseNumber(b){try{return b.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")}catch(d){return b}}
...
function getGameList(b){switch(b){case "BLITZ":b="Blitz Survival Games";break;case "BUILDBATTLE":b="Build Battle";break;case "COPSANDCRIMS":b="Cops and Crims";break;case "EASTER":b="Easter";break;case "GENERAL":b="General";break;case "MURDERMYSTERY":b="Murder Mystery";break;case "QUAKE":b="Quake";break;case "SPEEDUHC":b="Speed UHC";break;case "SUMMER":b="Summer";break;case "SUPERSMASH":b="Smash Heroes";break;case "TRUECOMBAT":b="Crazy Walls";break;case "WARLORDS":b="Warlords";break;case "WOOLGAMES":b=
"Wool Games"}return b}function getGame(b){return $gameTypes[b]&&$gameTypes[b].name||getGameList(b)}function getGameMode(b,d){return $gameTypes[b]&&$gameTypes[b].modeNames&&$gameTypes[b].modeNames[d]||d}function addCache(b,d){var f=getItem("cache");f[b]=d;localStorage.setItem("cache",JSON.stringify(f))}function isUUID(b){b=b.replace(/-/g,"");return/^[0-9a-f]{32}$/i.test(b)}$inputSettingsApiKey.on("input",function(){localStorage.setItem("apiKey",$inputSettingsApiKey.val())});
...
$buttonSettingsClearCache.click(function(){$alert("Confirmation","Are you sure about clearing cache?",function(){localStorage.setItem("cache",JSON.stringify({}));$("#globalPop button").unbind("click");$("#globalPop").popup("close");setTimeout(function(){$alert("Done","Cache Storage has been emptied")},1E3)},1)});
